...
import type { TopRetailerRow } from "@/lib/adminAnalytics";

export function TopRetailersCard({ rows }: { rows: TopRetailerRow[] }) {
  return (
    <DashboardCard
      icon={TrendingUp}
      title="Top Retailers"
      subtitle="By affiliate clicks"
      viewAllHref="/admin/retailers"
    >
      {rows.length === 0 ? (
        <p className="py-6 text-center text-sm text-espresso/45">No retailer clicks yet.</p>
      ) : (
